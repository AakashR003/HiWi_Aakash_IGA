...
class PrebucklingAnalysis:
    ...
    Compares buckling load/multiplier between symmetry model (quarter plate) and full model.
    ...
    def solve_prebuckling_problem(self, mp, num_nodes, iterations, echo_level=0):
    ...
    def set_up_system(self, num_nodes, length, symmetry):
        """Set up the complete system"""
        if symmetry:
            self.mp = self.current_model.CreateModelPart("Structure_Symmetry")
            print(f"  Setting up SYMMETRY model (quarter plate, {num_nodes}x{num_nodes} nodes, {length}x{length} m)")
        else:
        ...
        if symmetry:
            self.apply_bcs_symmetry(self.mp, num_nodes)
            # Loads on right edge
    ...
    def run(self, num_nodes, length, iterations, symmetry, echo_level=0):
        """Run the complete prebuckling analysis"""
        self.set_up_system(num_nodes, length, symmetry)
        print(f"  Solving prebuckling problem with {iterations} load steps...")
        load_factors = self.solve_prebuckling_problem(self.mp, num_nodes, iterations, echo_level)
        return load_factors


def print_results(load_factors_sym, load_factors_full, reference_value):
    ...
    print(f"  Symmetry Model (Quarter Plate):")
    print(f"    First load step:  λ = {load_factors_sym[0]:.4f}")
    print(f"    Last load step:   λ = {load_factors_sym[-1]:.4f}")
    print(f"    Variation:        {abs(1 - load_factors_sym[0] / load_factors_sym[-1]) * 100:.4f}%")
    
    print(f"\n  Full Model:")
    print(f"    Load multiplier:  λ = {load_factors_full[0]:.4f}")
    ...
    print("\n" + "-" * 70)
    print("Comparison:")
    error_full_vs_sym = abs(load_factors_full[0] - load_factors_sym[0]) / load_factors_sym[0] * 100
    error_sym_vs_ref = abs(load_factors_sym[0] - reference_value) / reference_value * 100
    error_full_vs_ref = abs(load_factors_full[0] - reference_value) / reference_value * 100
    
    print(f"  Full vs Symmetry:     {error_full_vs_sym:.4f}% difference")
    print(f"  Symmetry vs Reference: {error_sym_vs_ref:.2f}% error")
    print(f"  Full vs Reference:     {error_full_vs_ref:.2f}% error")
    
    print("\n" + "=" * 70)
    
    # Check convergence
    variation = abs(1 - load_factors_sym[0] / load_factors_sym[-1])
    if variation < 1e-4:
        print("✓ Load multiplier converged (variation < 0.01%)")
    else:
        print("✗ Warning: Load multiplier not fully converged")
    
    if abs(load_factors_full[0] - load_factors_sym[0]) / load_factors_sym[0] < 1e-4:
        print("✓ Symmetry and full models agree")
    else:
        print("✗ Warning: Symmetry and full models show differences")
    
    if abs(load_factors_sym[0] - reference_value) / reference_value < 0.01:
...
def main():
    ...
    print("  - Comparing quarter-plate (symmetry) vs full-plate models")
    print("=" * 70)
    
    # Reference value from ABAQUS
    reference_value = 92.80
    
    # Run symmetry model (quarter of the plate, 1x1 m)
    print("\n" + "=" * 70)
    print("RUNNING SYMMETRY MODEL (Quarter Plate)")
    print("=" * 70)
    num_nodes_sym = 5
    load_steps_sym = 18
    length_sym = 1
    
    analysis_sym = PrebucklingAnalysis()
    load_factors_sym = analysis_sym.run(num_nodes_sym, length_sym, load_steps_sym, symmetry=True, echo_level=1)
    ...
    load_factors_full = analysis_full.run(num_nodes_full, length_full, load_steps_full, symmetry=False, echo_level=1)
    
    # Print results
    print_results(load_factors_sym, load_factors_full, reference_value)
